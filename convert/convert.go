...
// TimeStrToInt64 convert timeStr to int64, 输出(例如)：1546926630
func TimeStrToInt64(ts string) int64 {
...
// TimeInt64ToStr convert int64 to timeStr, 输出(例如)：1546926630
func TimeInt64ToStr(ti int64) string {
	//时间转换的模板，golang里面只能是 "2006-01-02 15:04:05" （go的诞生时间）
	timeTemplate1 := "2006-01-02 15:04:05" //常规类型
	// timeTemplate2 := "2006/01/02 15:04:05" //其他类型
	// timeTemplate3 := "2006-01-02"          //其他类型
	// timeTemplate4 := "15:04:05"            //其他类型
	ts := time.Unix(ti, 0).Format(timeTemplate1)
	return ts
}
