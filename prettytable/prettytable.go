...
func NewTable(cols ...Column) (*Table, error) {
	...
	for i, c := range cols {
		...
		t.columns[i].width = runewidth.StringWidth(c.Header) + 1 // comment_1/4: +1 for a space reserved
...
func (t *Table) AddRow(vars ...interface{}) error {
	...
	for i, v := range vars {
		...
		row = append(row, s)
		strlen := runewidth.StringWidth(s) + 1 // comment_2/4: +1 for a space reserved
...
func (t *Table) Bytes() []byte {
	...
	addCell := func(i int, s string, max int) []byte {
		...
		if i >= 0 {
			cell.WriteString(t.Separator + " ") // comment_3/4: +" " for a space reserved,eg:| No. |
		}
	...
	addFrame := func(i int, s string, max int) []byte {
		var cell bytes.Buffer
		if i >= 0 {
			cell.WriteString("+-") // comment_4/4: +-, - for a space reserved
		}
		w := runewidth.StringWidth(s)
		sp := strings.Repeat(" ", t.columns[i].width-w)
		if t.columns[i].AlignRight {
