...
func TimeTrack(start time.Time, name string) {
	...
	logger.Infof("TimeTrack: %s elapsed %s", name, elapsed)
}
...
func GetFileSize(filename string) int64 {
	...
	logger.Debugf("File size: %s -> %d", filename, result)
	return result
}
...
func RunCmd(cmdSpc string) (rc int, output string, err error) {
	...
	if err != nil {
		logger.Warningf("RunCmd Failed: %s", fmt.Sprint(err))
		rc = -1
	}
...
// StrNumToIntArr .
func StrNumToIntArr(strNum, style string, expLen int) []int {
	numArr := []int{}
	for _, sn := range strings.Split(strNum, style) {
		in, _ := strconv.Atoi(sn)
		numArr = append(numArr, in)
	}
	numArrLen := len(numArr)
	exLen := expLen - numArrLen
	lastN := numArr[numArrLen-1]
	for i := 0; i < exLen; i++ {
		numArr = append(numArr, lastN)
	}
